    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->setupUi(this);
    QPixmap pix("C:/Users/leila/Desktop/new anis/Final1/Final1/Final/Final/final/animaux.png");
    int w=ui->label_5->width();
    int h=ui->label_5->height();
    ui->label_5->setPixmap(pix.scaled(w,h,Qt::KeepAspectRatio));
    QPixmap piix("C:/Users/leila/Desktop/new anis/Final1/Final1/Final/Final/final/nourritures.png");
    ui->label_6->setPixmap(piix);
    ...
    mysystem->showMessage(tr("notification"),tr("Nombre de vaccin à faire aujourd'hui : %1").arg(i));
...
void MainWindow::on_pushButton_supp_3_clicked()
{
    ...
    for(int i=0; i< list.count(); i++)
    {
        ...
        reply = QMessageBox::question(this, "Supprimer un animal", "Vous etes sure?",
...
void MainWindow::on_table_animaux_2_activated(const QModelIndex &index)
{

    id_animaux= ui->table_animaux_2->model()->data(index).toInt();
    QSqlQuery query;
    query.prepare("select * from animaux where id_animaux= :id ");
    query.bindValue(":id", id_animaux);
    if(query.exec())
    {
        while(query.next())
        {
...
void MainWindow::on_lineEdit_3_textChanged(const QString &index)
{
    QString str=ui->lineEdit_3->text();
    ui->table_animaux_2->setModel(tmp.cherchere(str));
...
void MainWindow::on_ajout_nourr_3_clicked()
{
    ...
    nourritures n(90,  nom, quantite_nour, prix_unitaire);
    QMessageBox msg;
    if(n.ajouter())
    {
        ui->table_nourritures_4->setModel(tmpn.afficher());//refresh
        mysystem->showMessage(tr("notification"),tr("produit ajouté avec succes"));
        msg.setText("ajout avec succès");
        msg.exec();
    }
    else
...
void MainWindow::on_supp_nourr_3_clicked()
{
    ...
    for(int i=0; i< list.count(); i++)
    { QMessageBox::StandardButton reply;
        reply = QMessageBox::question(this, "Supprimer un produit", "Vous etes sure?",
                                      QMessageBox::Yes|QMessageBox::No);
        if (reply == QMessageBox::Yes) {
        ...
        ui->table_nourritures_4->setModel(tmpn.afficher());
        mysystem->showMessage(tr("notification"),tr("produit supprimé avec succes"));
    }
...
void MainWindow::on_table_nourritures_4_activated(const QModelIndex &index)
{
    id_nourriture= ui->table_nourritures_4->model()->data(index).toInt();
    QSqlQuery query;
    query.prepare("select * from nourritures where id_nourriture= :id ");
    query.bindValue(":id", id_nourriture);
    ...
    if(query.exec())
    {
        while(query.next())
        {
            ...
            ui->le_nom_3->setCurrentText(query.value(1).toString());
            ui->la_qt_3->setText(query.value(2).toString());
            ui->le_prix_un_3->setText(query.value(3).toString());
...
{
    ui->table_nourritures_4->setModel(tmpn.triee(0));
}
void MainWindow::on_pushButton_22_clicked()
{
    ui->table_nourritures_4->setModel(tmpn.triee(1));
}
void MainWindow::on_lineEdit_textChanged(const QString &arg1)
{
    QString str=ui->lineEdit->text();
    ui->table_nourritures_4->setModel(tmpn.cherchere(str));
}
...
void MainWindow::on_pushButton_99_clicked()
{
    ...
    const int rowCount = ui->table_nourritures_4->model()->rowCount();
    const int columnCount = ui->table_nourritures_4->model()->columnCount();
    QString TT = QDate::currentDate().toString("yyyy/MM/dd");
    out <<"<html>\n"
          "<head>\n"
          "<meta Content=\"Text/html; charset=Windows-1251\">\n"
    ...
    out << "<thead><tr bgcolor=#d6e5ff>";
    for (int column = 0; column < columnCount; column++)
        if (!ui->table_nourritures_4->isColumnHidden(column))
            out << QString("<th>%1</th>").arg(ui->table_nourritures_4->model()->headerData(column, Qt::Horizontal).toString());
    out << "</tr></thead>\n";
    ...
    for (int row = 0; row < rowCount; row++) {
        ...
        for (int column = 0; column < columnCount; column++) {
            if (!ui->table_nourritures_4->isColumnHidden(column)) {
                QString data =ui->table_nourritures_4->model()->data(ui->table_nourritures_4->model()->index(row, column)).toString().simplified();
                out << QString("<td bkcolor=0>%1</td>").arg((!data.isEmpty()) ? data : QString("&nbsp;"));
            }
        }
        out << "</tr>\n";
...
void MainWindow::on_pb_ajoutern_clicked()
{
    ...
    if(ui->lineEdit_CIN->text() == "" || ui->lineEdit_nom->text() == "" || ui->lineEdit_tel->text() == "")
    {
        QMessageBox::about(this,"Empty values!","Please Enter Values to be Add!");
    }
    else if(ui->lineEdit_nom->text().length() < 3)
    {
        QMessageBox::about(this,"Wrong values!","Please Enter your correct name to be added!");
    }
    else if(ui->lineEdit_nom->text().contains("1") || ui->lineEdit_nom->text().contains("2") || ui->lineEdit_nom->text().contains("3") || ui->lineEdit_nom->text().contains("4") || ui->lineEdit_nom->text().contains("5") || ui->lineEdit_nom->text().contains("6") || ui->lineEdit_nom->text().contains("7") || ui->lineEdit_nom->text().contains("8") || ui->lineEdit_nom->text().contains("9") || ui->lineEdit_nom->text().contains("0"))
    {
        QMessageBox::about(this,"Wrong values!","Numbers are not allowed !");
    }
    else if(ui->lineEdit_CIN->text().length() < 8 || ui->lineEdit_CIN->text().length() > 8)
    {
        QMessageBox::about(this,"Wrong values!","Invalid CIN!");
    }
    else if(ui->lineEdit_tel->text().length() < 8 || ui->lineEdit_tel->text().length() > 8)
    {
...
void MainWindow::on_pb_ajouter_clicked()
{
    ...
    if(test)
    {
        ...
        ui->comboBox_2->setModel(tmpvet.affichervet());
        QSqlQueryModel *model = new QSqlQueryModel;
        model->setQuery("select NOM from veterinaire");
        ui->comboBox_2->setModel(model);
        QMessageBox::information(nullptr, QObject::tr("Ajouter un vaccin"),
...
void MainWindow::on_pb_ajoutervet_clicked()
{
    ...
    QString nom= ui->lineEdit_nom_2->text();
    QString CIN= ui->lineEdit_CIN_2->toPlainText();
...
//modif vet
void MainWindow::on_pushButton_ner_clicked()
{
    QString nom= ui->lineEdit_nom_2->text();
    QString CIN= ui->lineEdit_CIN_2->toPlainText();
    int prix= ui->SpinBox_prix->text().toInt();
